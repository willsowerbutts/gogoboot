...
{
    // if a processing function takes the packet from us it returns true, so we
    // know we shouldn't free the packet ourselves.
    bool taken = false;

    if(memcmp(packet->eth->destination_mac, macaddr_interface, sizeof(macaddr_t)) == 0 ||
       memcmp(packet->eth->destination_mac, macaddr_broadcast, sizeof(macaddr_t)) == 0){
        /* ooh! */
            ...
            case ethertype_ipv4:
                ...
                switch(packet->ipv4->protocol){
                    case ip_proto_tcp:
                        ...
                        // set packet->userdata...
                        // packet->user_data = packet->tcp->options+...;
                        break;
                    case ip_proto_udp:
