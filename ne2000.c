...
dp83902a_send(unsigned char *data, int total_len, unsigned long key)
{
        ...
        DP_OUT(base, DP_ISR, DP_ISR_RDC);  /* Clear end of DMA */
        {
                ...
                cyg_uint16 __attribute__((unused)) tmp;
                int len = 1;
...
dp83902a_RxEvent(void)
{
        struct dp83902a_priv_data *dp = (struct dp83902a_priv_data *) &nic;
        cyg_uint8 *base = dp->base;
        unsigned char __attribute__((unused)) rsr;
...
dp83902a_TxEvent(void)
{
        struct dp83902a_priv_data *dp = (struct dp83902a_priv_data *) &nic;
        cyg_uint8 *base = dp->base;
        unsigned char __attribute__((unused)) tsr;
        unsigned long key;
...
dp83902a_ClearCounters(void)
{
        struct dp83902a_priv_data *dp = (struct dp83902a_priv_data *) &nic;
        cyg_uint8 *base = dp->base;
        cyg_uint8 __attribute__((unused)) cnt1, cnt2, cnt3;
...
static const hw_info_t hw_info[] = {
...
static void pcnet_reset_8390(void)
{
        ...
        r = n2k_inb(nic_base + PCNET_RESET);
        delay_ms(10); //// WRS try this???
        n2k_outb(r, nic_base + PCNET_RESET);
        delay_ms(10); //// WRS try this???
        ...
        n2k_outb(0xff, EN0_ISR); /* Ack all interrupts */

        if (i == 100)
                printf("pcnet_reset_8390() did not complete.\n");
        printf("pcnet_reset_8390: ok?\n");
} /* pcnet_reset_8390 */
...
static const hw_info_t * get_prom(void ) {
    ...
    for (i = 0; i < sizeof(get_prom_program_seq)/sizeof(get_prom_program_seq[0]); i++)
        n2k_outb(get_prom_program_seq[i].value, get_prom_program_seq[i].offset);

    PRINTK("PROM:");
    for (i = 0; i < 32; i++) {
        prom[i] = isa_read_byte(PCNET_DATAPORT+CONFIG_DRIVER_NE2000_BASE);
        PRINTK(" %02x", prom[i]);
    }
    PRINTK("\n");
...
static unsigned char pbuf[NB * MAXPKTLEN]; 
static int plen[NB];
static int nrx = 0;
...
void uboot_push_packet_len(int len) {
        ...
        plen[nrx] = len;
        dp83902a_recv(&pbuf[nrx*MAXPKTLEN], len);
        nrx++;
}
...
void NetReceive(unsigned char *packet, int packet_length)
{
    printf("NetReceive(0x%x, %d)\n", (int)packet, packet_length);
    pretty_dump_memory(packet, packet_length);
}
...
int eth_rx(void) {
        ...
        tmo = set_timer_sec(TOUT);

        while(1) {
                dp83902a_poll();
                if (nrx > 0) {
                        for(j=0; j<nrx; j++) {
                                NetReceive(&pbuf[j*MAXPKTLEN], plen[j]);
                        }
                        nrx = 0;
                        return 1;
                }
                //if (get_timer (0) >= tmo) {
                if (timer_expired(tmo)){
...
int eth_send(void *packet, int length, unsigned long key) {
        ...
        dp83902a_send((unsigned char *) packet, length, key);
        //tmo = get_timer (0) + TOUT * CFG_HZ;
        tmo = set_timer_sec(TOUT);
        while(1) {
                dp83902a_poll();
